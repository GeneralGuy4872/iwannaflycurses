dijkstra(ortho,mode,runningnorm,tile,pointa,pointb,fly,swim)
bool ortho
schar mode
uchar runningnorm
schar tile
...
setcoord3* coordreturn_head = coordreturn_tail
setcoord3* coordreturn_new
coord3 nextcoord
...
while (!((currentcoord.x == pointa.x) && (currentcoord.y == pointa.y) && (currentcoord.z == pointa.z))) {
	...
	if (!datakeeper→grid[COORDSUB(currentcoord.z,currentcoord.y,currentcoord.x)].ex) {
		coordreturn_new = malloc(sizeof(setcoord3))
		coordreturn_head→prev = coordreturn_new
		coordreturn_tail→next = coordreturn_new
		coordreturn_head = coordreturn_new
		}
	}
return coordreturn_head
}